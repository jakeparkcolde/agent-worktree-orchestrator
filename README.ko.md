...
이 명령은 설정된 기준 참조와 워크트리 수를 확인한 뒤 명시된 목표로 독립 Orca 작업자를 요청합니다. `--no-parent`는 Orca의 부모·자식 관계를 제어하며, Git 시작점은 설정된 기준 참조를 사용합니다. 작업자가 준비되었거나 실행 중이라고 보고하기 전에 반환된 배분 결과를 확인하세요.

기본 체크아웃은 다른 작업도 배분할 수 있도록 총괄 에이전트의 조율 공간으로
유지합니다. 구현은 정확한 작업 워크트리에 연결된 **별도 Orca 에이전트 터미널**에서
진행하고 목표를 명시적으로 전달합니다. **워크트리 생성**, **세션 준비**, **작업 턴 시작**은
서로 다른 상태입니다. 중간에 실패하면 워크트리를 보존하고 연결을 복구하며,
총괄 에이전트가 해당 폴더로 이동해 대신 구현하는 자동 대체 흐름은 사용하지 않습니다.
[Orca 작업 배분](docs/orca-integration.md)에서 상태 확인과 재사용 방법을 확인하세요.
기존 워크트리에 터미널이 없으면 선택한 Codex/Claude를 별도로 시작합니다.
종류를 확인할 수 없는 터미널이 있으면 기본적으로 중단합니다. 작업자가 없는
일반 셸뿐임을 사용자가 알고 있을 때만 `--worktree PATH --new-session`을
명시하세요. 에이전트가 확인된 경우에는 이 옵션으로도 중복 실행하지 않습니다.
...
동등성 자동 인증은 후속 과제입니다. `request`는 기록된 같은 목표의
워크트리를 재사용합니다. 목표의 의미 판단, 수정 파일 중복 조율, 설정한
테스트 실행, PR 생성과 병합은 사람 또는 총괄 에이전트가 맡습니다.

[v0.2 로드맵](ROADMAP.md)과 [변경 이력](CHANGELOG.md)을 확인하세요.

## 자연어로 작업 시작하기

총괄 에이전트가 “AWO 카카오 비서 관련 작업하고 싶다”라는 요청을 받으면
`awo request`로 등록된 프로젝트 이름·별칭과 기존 worktree를 확인합니다.
구체적인 목표가 없으면 질문하며, 확인한 목표를 `--goal`로 전달합니다.

```bash
./bin/awo request 'AWO 카카오 비서' --goal '미답 질문 오탐 수정'
./bin/awo request 'AWO 카카오 비서' --goal '미답 질문 오탐 수정' --apply
```

첫 호출은 미리보기입니다. `--apply`는 같은 목표의 worktree를 재사용하거나
설정된 `base_ref`에서 Orca `--no-parent`로 생성합니다. 미등록 기존 worktree가
있으면 먼저 목표를 확인해야 합니다. 기본값은 현재 에이전트가 이어서 작업하는
`--agent none`입니다. [별칭 등록과 거점 연동](docs/request-entry.md)을 참고하세요.

애매한 요청에 `--advise jev`를 붙이면 프로젝트·기존 목표·요청 유형 후보를 추천합니다.
규칙 결정은 유지하며 `--apply`에서는 추천 호출을 생략합니다.
[전송 범위·키 설정·평가·롤백 안내](docs/jev-advice.md)를 참고하세요.
